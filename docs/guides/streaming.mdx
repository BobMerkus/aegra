...
<Note>
  `debug` mode is automatically added to every stream — you don't need to request it explicitly. `updates` mode is also added internally to detect interrupt events for human-in-the-loop support.
</Note>
